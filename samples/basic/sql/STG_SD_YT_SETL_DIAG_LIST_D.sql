INSERT OVERWRITE TABLE STG_SD_YT_SETL_DIAG_LIST_D PARTITION(pt)
SELECT  '1' AS MAINDIAG_FLAG    --主诊断标志  地纬默认'1'--1
        ...
        ,(
            SELECT dise_name FROM pub_ytai_data.stg_tb_dise_list_b_df_v_ytai dise WHERE dise.DISE_CODE = mdtrt.dise_code LIMIT 1
        ) AS DIAG_NAME    --诊断名称--9
        ...
        ,nvl(
            (
                SELECT admdvs FROM pub_ytai_data.stg_gb_optins_info_b_df_v_ytai c WHERE length(c.optins_no)=6 AND mdtrt.poolarea = c.optins_no LIMIT 1
            )
            ,'370600'
        ) AS OPTINS_NO    --经办机构编号
        ,nvl(
            ...
            ,'370600'
        ) AS POOLAREA_NO    --统筹区编号
        ,NULL AS EVT_INST_ID    --事件实例ID
        ,mdtrt.pt AS pt
FROM    pub_ytai_data.stg_tp_mdtrt_d_v_ytai mdtrt
LEFT JOIN pub_ytai_data.stg_pi_psn_info_b_df_v_ytai b
...
INSERT INTO TABLE STG_SD_YT_SETL_DIAG_LIST_D PARTITION(pt)
SELECT  '1' AS MAINDIAG_FLAG    --主诊断标志  地纬默认'1'--1
        ,setl.MDTRT_ID AS DIAG_INFO_ID    --2
        ,nvl(
            (
                SELECT mdtrt_id_nat FROM pub_ytai_data.stg_tp_mdtrt_d_v_ytai ts WHERE ts.mdtrt_id = setl.mdtrt_id LIMIT 1
            )
            ,setl.MDTRT_ID
        ) AS MDTRT_ID    --3
        ...
        ,(
            SELECT dise_name FROM pub_ytai_data.stg_tb_dise_list_b_df_v_ytai dise WHERE dise.DISE_CODE = setl.dise_code LIMIT 1
        ) AS DIAG_NAME    --诊断名称
        ...
        ,nvl(
            (
                SELECT admdvs FROM pub_ytai_data.stg_gb_optins_info_b_df_v_ytai c WHERE length(c.optins_no) = 6 AND setl.poolarea = c.optins_no LIMIT 1
            )
            ,'370600'
        ) AS OPTINS_NO    --经办机构编号
        ,nvl(
        ...
        ,NULL AS EVT_INST_ID    --事件实例ID
        ,setl.pt AS pt
FROM    pub_ytai_data.stg_tp_setl_d_v_ytai setl
LEFT JOIN pub_ytai_data.stg_pi_psn_info_b_df_v_ytai b
ON      setl.psn_no = b.psn_no
WHERE   setl.insutype IN ('C', 'E')
AND     b.certno IS NOT NULL
AND     setl.dise_code IS NOT NULL
AND     NOT EXISTS (SELECT 1 FROM sc_dr_ytai_sjzq.STG_SD_YT_SETL_DIAG_LIST_D mdtrt WHERE mdtrt.mdtrt_id = setl.mdtrt_id AND mdtrt.diag_code IS NOT NULL AND mdtrt.diag_code <> setl.dise_code)
;
--结算清单表         
...
SELECT  '1' AS MAINDIAG_FLAG    --主诊断标志  地纬默认'1'
        ...
        ,nvl(
            (
                SELECT mdtrt_id_nat FROM pub_ytai_data.stg_tp_mdtrt_d_v_ytai ts WHERE ts.mdtrt_id = setlList.mdtrt_id LIMIT 1
            )
            ,setlList.MDTRT_ID
        ) AS MDTRT_ID
        ...
        ,NULL AS OPTER_NAME    --经办人姓名
        ,NULL AS OPT_TIME    --经办时间
        ,setlList.admdvs AS OPTINS_NO    --经办机构编号
        ,setlList.admdvs AS POOLAREA_NO    --统筹区编号
        ,NULL AS EVT_INST_ID    --事件实例ID
        ,setlList.pt AS pt
FROM    pub_ytai_data.stg_setl_mdcs_fund_setl_list_diag_d_v_ytai setlList
LEFT JOIN pub_ytai_data.stg_pi_psn_info_b_df_v_ytai b
ON      setlList.psn_no = b.psn_no
WHERE   b.certno IS NOT NULL
AND     setlList.diag_code IS NOT NULL
AND     NOT EXISTS (SELECT 1 FROM sc_dr_ytai_sjzq.STG_SD_YT_SETL_DIAG_LIST_D mdtrt WHERE mdtrt.mdtrt_id = setlList.mdtrt_id AND mdtrt.diag_code IS NOT NULL AND mdtrt.diag_code <> setlList.diag_code)
;
