...
def process_page_workflow(args):
    ...
    # System Prompt：确立“语言学教材数字化专家”的人设
    system_prompt = "你是一个专业的语言学教材排版与内容数字化专家，特别是英语语法与长难句分析领域的专家。你的核心任务是完美还原文档内容，智能区分文本转换与图像保留，并确保输出格式清晰、结构化，适合阅读和学习。"
    
    # 优化后的 User Prompt：适配英语语法书
    user_prompt = (
        "请对这张教材图片进行布局分析与内容转换，目标是生成完美的 Markdown 笔记，严格遵循以下核心指令：\n\n"
        
        "### 1. 智能分流策略 (防止切图过碎)\n"
        "- **文本优先**：绝大多数内容（包括中文讲解、英文例句、语法规则、表格）**必须**转换为 Markdown 文本。\n"
        "- **仅大图切割**：只有遇到**无法用代码描述**的复杂图形（如复杂的思维导图、插画、漫画场景），才输出切割指令。\n"
        "- **禁止事项**：**严禁**将行内的单词、标点、简单的框图切成图片。\n\n"
        ...
        "### 3. 语言学内容排版规范\n"
        "- **中英对照**：对于出现的中英文对照句子，请保持清晰的对应关系。如果是逐行对照，请按行输出；如果是段落对照，请分段输出。\n"
        "- **例句格式**：\n"
        "  - 英文例句建议使用引用块 `>` 或者代码块（如果需要强调结构）包裹，或者直接加粗。\n"
        "  - 例句中的**关键语法点**（如加粗、斜体、下划线部分）必须保留 Markdown 格式（**Bold**, *Italic*）。\n"
        "- **语法结构分析**：\n"
        "  - 如果有句子成分分析（如主谓宾划分），请尝试用文字描述（例如：“主语：... 谓语：...”），或者使用简单的 Markdown 表格。\n"
        "  - 遇到长难句分析图解，如果能用文字清晰表述逻辑，优先用文字；非常复杂无法表述时，使用切图。\n\n"
        
        "### 4. 标题与层级\n"
        "- **标题还原**：准确识别章节标题（Chapter/Section），并使用 Markdown 的 `#`, `##`, `###` 对应。\n"
        "- **列表与序号**：保留原书的列表结构（1. 2. 3. 或 a. b. c.），确保缩进正确。\n\n"

        "### 5. 试题与练习\n"
        "- **练习题**：\n"
        "  - 题号加粗。\n"
        "  - 填空题的横线统一使用 `______`。\n"
        "- **解析**：如果有答案解析，请将其与题目适当分开，或使用折叠块 `<details>`（如果适合）或引用块展示。\n\n"
        
        "### 6. 强力去噪与纯净输出\n"
        "- **去广告**：绝对禁止输出“关注公众号”、“免费资料”等水印。\n"
        "- **去干扰**：忽略页眉、页脚、页码。\n"
...
def main():
    ...
    # 修改为可选参数，并设置默认值
    default_pdf = r"ai\files2\2024刘晓艳大雁教你语法长难句.pdf"
    parser.add_argument("pdf_path", nargs='?', default=default_pdf, help=f"PDF文件路径 (默认: {default_pdf})")
    args = parser.parse_args()

    if not os.path.exists(args.pdf_path):
        # 尝试使用相对路径拼接
        if os.path.exists(os.path.join(os.getcwd(), args.pdf_path)):
            args.pdf_path = os.path.join(os.getcwd(), args.pdf_path)
        else:
