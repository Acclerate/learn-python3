...
def process_page_workflow(args):
    ...
    user_prompt = (
        ...
        "### 3. LaTeX 基础规范 (Obsidian 兼容基石)\n"
        "- **公式包裹**：**所有**数学表达式（包括简单的变量 $x$, 数字 $1$, 极限 $\\lim$）**必须**包裹在 `$` (行内) 或 `$$` (独占行) 中。\n"
        "- **禁止代码块**：数学公式**严禁**使用反引号（`）包裹。错误示范：`$x$`，正确示范：$x$。\n"
        "- **中文混排**：公式内的中文建议包裹在 `\\text{...}` 中。\n"
        "- **转义规范**：集合或分段函数的大括号必须转义 `\\{` `\\}`；换行必须使用双反斜杠 `\\\\`。\n\n"

        "### 4. 高级数学结构排版 (解决乱码与挤压)\n"
        "- **分段函数 (重中之重)**：**必须**使用 `\\begin{cases}...\\end{cases}` 环境。**严禁**使用 `array`。\n"
        "  - 并且：整个分段函数必须包裹在 **块级公式** `$$...$$` 中，禁止使用单 `$ `。\n"
        "- **表格与矩阵 (简化结构)**：\n"
        "  - **必须**使用 `\\begin{array}...\\end{array}` 环境。**严禁**使用 Markdown 表格语法（|---|）。\n"
        "  - **禁用复杂命令**：**严禁**使用 `\\multirow` (多行合并) 和 `\\cline` (部分横线)，因为 Obsidian 不支持。\n"
        "  - **处理策略**：遇到合并单元格时，请将其拆分为标准单元格并填入对应内容，或保持简单的行列网格结构。\n"
        "- **复杂公式块级化**：凡是包含 `\\lim` (极限)、`\\frac` (分数)、`\\int` (积分) 的算式，**必须**包裹在 `$$ ... $$` 中独占一行。\n"
        "  - 正确示范：$$ \\lim_{x \\to 0} \\frac{\\sin x}{x} = 1 $$\n\n"
        
        "### 5. 试题排版规范 (填空与选择)\n"
        "- **填空题**：\n"
        "  - 题号加粗：如 **1.6**。\n"
        "  - 横线统一：使用 `\\text{\\underline{\\hspace{1cm}}}` 或 `______`。\n"
        "- **选择题选项**：\n"
        "  - **方案 A（推荐 - 竖排列表）**：默认将选项转换为 Markdown 列表。\n"
        "    - (A) $x=1$\n"
        "    - (B) $x=2$\n"
        "  - **方案 B（仅当必须横排时 - 全包裹模式）**：必须将**整行内容**包裹在 `$$` 中，内部用 `\\quad` 分隔。\n"
