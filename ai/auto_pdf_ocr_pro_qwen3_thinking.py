...
def process_page_workflow(args):
    ...
    # 优化后的 User Prompt：集成了【智能分流】、【切图指令】、【排版规范】和【强力去广】
    user_prompt = (
        "请对这张教材图片进行布局分析与内容转换，严格遵循以下四条核心指令：\n\n"
        
        "### 1. 智能分流策略（关键）\n"
        "- **直接转换**：对于文本、数学公式、简单表格，请直接转换为 Markdown 和 LaTeX 格式。\n"
        "- **保持原图**：对于函数图像、几何图形、复杂的竖式计算、手写笔记或难以用代码还原的逻辑图，**不要**尝试描述，**不要**转文字。\n"
        "  -> 请直接输出切割指令标签（见下条）。\n\n"
        
        "### 2. 切割指令格式\n"
        "- 在图片应该插入的位置，输出标签：`<CUT_IMG>xmin,ymin,xmax,ymax</CUT_IMG>`\n"
        "- 坐标系要求：使用 [0, 1000] 的归一化坐标（左上角0,0，右下角1000,1000）。\n"
        "- 排版要求：该标签必须**单独占一行**，不要夹杂在文本段落中。\n\n"
        
        "### 3. 排版与数学规范\n"
        "- **公式**：行内公式用 $...$，独占行公式用 $$...$$。\n"
        "- **表格**：必须还原结构，使用 LaTeX array 或 Markdown 表格，禁止乱码。\n\n"
        
        "### 4. 强力去噪与负面约束\n"
        "- **去广告**：绝对禁止输出“关注公众号【考研小舟】”、“免费考研资料&无水印PDF”等水印内容。\n"
        "- **去干扰**：忽略页眉、页脚、页码、页面角落的二维码。\n"
        "- **纯净输出**：直接输出最终的 Markdown 内容，不要输出“好的”、“分析如下”等任何废话。"
    )
