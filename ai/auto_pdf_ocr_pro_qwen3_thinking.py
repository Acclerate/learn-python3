...
def process_page_workflow(args):
    ...
    # User Prompt：深度优化版
    user_prompt = (
        "请对这张教材图片进行布局分析与内容转换，严格遵循以下五大核心指令：\n\n"
        
        "### 1. 智能分流策略\n"
        "- **文本处理**：中文解释性文字使用 Markdown。\n"
        "- **数学与表格**：**所有**数学公式、变量（如 $x$）、数字（如 $1$）、表格必须使用 LaTeX 格式。\n"
        "- **复杂图形**：函数图像、几何图形、竖式计算、手写笔记等，**不要**尝试描述，**不要**转文字，直接输出切割指令。\n\n"
        ...
        "- **中文混排**：公式内的中文必须包裹在 `\\text{...}` 中。\n\n"
        "- **表格**：必须使用 `\\begin{array}...\\end{array}` 环境，换行用 `\\\\`，大括号转义为 `\\{`。\n"
        "- **禁止代码块**：数学公式**严禁**使用反引号（`）包裹。错误：`$x$`，正确：$x$。\n\n"   
